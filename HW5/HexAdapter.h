...
/*	This class is derived class from AbstractHex class.	*/
	...
	public: 
		HexAdapter(): HexAdapter(6,1){/*Intentionally empty*/	} //default constructor
		HexAdapter(int boardSize, int user);	//constructor with two parameter
		HexAdapter(int board_size, int user, string board_data); //load komutu için (computer-user)
		HexAdapter(int board_size, int user, string board_data, int user_turn); //load komutu için (player1-player2)
		HexAdapter(const HexAdapter& temp);
