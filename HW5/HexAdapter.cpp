...
/*	Constructor with two parameter	*/
	...
	HexAdapter<T> :: HexAdapter (int boardSize, int user){
		cout<<"HexAdapter CLASS CONST2"<<endl;
			if(boardSize<=5){		//checks validity of entered board size
				...
				cin>>boardSize;
				HexAdapter(boardSize,user);	//enter new board size
			}else
				setSize(boardSize); 
			if(user!=1 && user!=2){
		...
		template<template<typename...>class T>
		HexAdapter<T> ::HexAdapter(int board_size, int user, string board_data){
			setSize(board_size);
			setPlayer(user);
			setBoard(board_data);
		}
		...
		template<template<typename...>class T>
		HexAdapter<T>::HexAdapter(int board_size, int user, string board_data, int user_turn) : turn(user_turn){
			setSize(board_size);
			setPlayer(user);
			setBoard(board_data);
		}
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: setSize(int boardSize){
			size= boardSize;
			reset();
			allMoves.resize(getSize(), vector<int>(getSize()));
			for(int i=0; i<size; i++)
		...
		void HexAdapter<T>::setBoard(){
			hexCells.resize(getSize(), T<Cell>(getSize()));
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: setBoard(string board_data){
			reset();
			hexCells.resize(getSize(), T<Cell>(size));
			int k=0;
			char temp;
			for(int i=0; i<getSize(); i++){ 
				for(int j = 0; j < getSize(); j++){
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: print(string finish){
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: enterCommand(){
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: analyze_command(string command){
	...
	/*	It's an utility function to check the coordinates valid or not. Include uppercase and lowercase conditions. Returns 0 if invalid, returns 1 or 2 if valid */
		template<template<typename...>class T>
		void HexAdapter<T>:: coordinates_valid_or_not(){
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: make_move(int flag){
	...
	*/
	...
	/* This function generates random coordinates for computer's move*/
		template<template<typename...>class T>
		void HexAdapter<T>:: choose_different_rand(int &last_x, int &last_y){
	...
	/* This function makes computer's move on the board*/
		template<template<typename...>class T>
		int HexAdapter<T> :: computerMove ( int first ){ 
	...
	rand_again:
		...
		template<template<typename...>class T>
		void HexAdapter<T>::game_end(int won){
				if(won=='x'){
					print("finish");
	...
	template<template<typename...>class T>
	int HexAdapter<T>:: check_finish_x(){
...
check:			
	...
	template<template<typename...>class T>
	int HexAdapter<T>:: check_finish_o(){
			board_won.resize(size, vector<int>(size));
			int counter=1, x=0, y=0,i,j;
	...
	check:					
		...
		template<template<typename...>class T>
		bool HexAdapter<T>:: isEnd(int player){
			if(player == 1){
				if(check_finish_x() == 1)
					return true;
				return false;
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: make_uppercase( char letter_won){
			int i, j;
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: writeToFile(string fileName){
		...
		template<template<typename...>class T>
		void HexAdapter<T>:: readFromFile(string fileName){
			...
			int user_turn;
			if(player==1)
				HexAdapter(size,player,board_data);			
				...
				HexAdapter(size, player, board_data, user_turn);			
			}
		}
		...
		template<template<typename...>class T>
		char HexAdapter<T>::operator() (int i1, int i2){
			try{
		...
		template<template<typename...>class T>
		bool HexAdapter<T>:: operator==( AbstractHex& other){
			 HexAdapter<T>* cast_other = static_cast< HexAdapter<T>*> (&other);
